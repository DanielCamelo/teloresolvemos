...
const DetalleTransporteParticular = () => {
    ...
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(
          SummaryApi.getTransporteParticularByUserIdAndOrderId.url(orderId),
          {
            method: SummaryApi.getTransporteParticularByUserIdAndOrderId.method,
  ...
  doc.addImage(logo, "PNG", logoX, logoY, logoWidth, logoHeight);
    ...
    // Encabezado
    doc.setFontSize(18);
    doc.text("ORDEN DE TRANSPORTE PUBLICO", pageWidth / 2, 20, { align: "center" });
    doc.setFontSize(12);
    ...
    // Rellenar tabla
    doc.setFont("helvetica", "normal");
    doc.text("Servicio de Transporte Particular", 15, tableY + 17);
    doc.text(
    ...
    // Guardar el archivo
    doc.save(`detalle_Transporte_Particular_${order._id || "orden"}.pdf`);
  };
...
export default DetalleTransporteParticular