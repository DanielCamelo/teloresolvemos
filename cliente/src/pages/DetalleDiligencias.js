...
const DetalleDiligencias = () => {
    ...
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(
          SummaryApi.getDiligenciasByUserIdAndOrderId.url(orderId),
          {
            method: SummaryApi.getDiligenciasByUserIdAndOrderId.method,
  ...
  doc.addImage(logo, "PNG", logoX, logoY, logoWidth, logoHeight);
    ...
    // Encabezado
    doc.setFontSize(18);
    doc.text("ORDEN DE DILIGENCIAS", pageWidth / 2, 20, { align: "center" });
    doc.setFontSize(12);
    ...
    currentY += lineSpacing;
    doc.text(`Descripcion de diligencia: ${order.descripcionDiligencia} `, 10, currentY);
    currentY += lineSpacing;
    doc.text(`Direcciones involucradas: ${order.direccionInvolucrados}`, 10, currentY);
    currentY += lineSpacing;
    ...
    // Rellenar tabla
    doc.setFont("helvetica", "normal");
    doc.text("Servicio de diligencia", 15, tableY + 17);
    doc.text(
    ...
    // Guardar el archivo
    doc.save(`detalle_diligencia_${order._id || "orden"}.pdf`);
  };
      ...
      {loading ? (
        ...
        <div>
          <div className="flex justify-between items-center border-b pb-3">
            <h2 className="text-xl font-bold">{order.descripcionDiligencia}</h2>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-gray-700">Direcciónes involucradas:</span>
              <span className="text-right">{order.direccionInvolucrados}</span>
            </div>
...
export default DetalleDiligencias