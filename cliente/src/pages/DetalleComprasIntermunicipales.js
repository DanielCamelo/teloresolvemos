...
const DetalleComprasIntermunicipales = () => {
    ...
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(
          SummaryApi.getComprasIntermunicipalesByUserIdAndOrderId.url(orderId),
          {
            method: SummaryApi.getComprasIntermunicipalesByUserIdAndOrderId.method,
  ...
  doc.addImage(logo, "PNG", logoX, logoY, logoWidth, logoHeight);
    ...
    // Encabezado
    doc.setFontSize(18);
    doc.text("ORDEN DE COMPRAS INTERMUNICIPALES", pageWidth / 2, 20, { align: "center" });
    doc.setFontSize(12);
    ...
    doc.text(`Fecha: ${new Date(order.fechaHoraEntrega).toLocaleDateString()}`, 10, currentY);
    currentY += lineSpacing;
    ...
    currentY += lineSpacing;
    doc.text(`Productos: ${order.productos} `, 10, currentY);
    currentY += lineSpacing;
    doc.text(`Dirección de compras: ${order.ubicacionCompra}`, 10, currentY);
    currentY += lineSpacing;
    ...
    // Rellenar tabla
    doc.setFont("helvetica", "normal");
    doc.text("Servicio de compras intermunicipales", 15, tableY + 17);
    doc.text(
        order.precio !== null ? `$${order.precio}` : "Precio no asignado",
        125,
        tableY + 17
      );
    ...
    // Guardar el archivo
    doc.save(`detalle_compras_intermunicipales_${order._id || "orden"}.pdf`);
  };
      ...
      {loading ? (
        ...
        <div>
          <div className="flex justify-between items-center border-b pb-3">
            <h2 className="text-xl font-bold">{order.ubicacionCompra}</h2>
            <span className="text-sm text-gray-500">
              {new Date(order.fechaHoraEntrega).toLocaleDateString()}
            </span>
          </div>
    ...
    >
      ...
      <span className="text-xs">Cancelado</span>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-gray-700">Presupuesto maximo:</span>
              <span>{order.presupuestoMaximo}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-700">Dirección de compra:</span>
              <span className="text-right">{order.ubicacionCompra}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-gray-700">{order.precio !== null ? `$${order.precio}` : "Sin asignar"}</span>
...
export default DetalleComprasIntermunicipales