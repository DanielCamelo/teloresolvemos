...
const DetalleTransporteSalud = () => {
    ...
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(
          SummaryApi.getTransporteSaludByUserIdAndOrderId.url(orderId),
          {
            method: SummaryApi.getTransporteSaludByUserIdAndOrderId.method,
  ...
  doc.addImage(logo, "PNG", logoX, logoY, logoWidth, logoHeight);
    ...
    // Encabezado
    doc.setFontSize(18);
    doc.text("ORDEN DE TRANSPORTE DE SALUD", pageWidth / 2, 20, { align: "center" });
    doc.setFontSize(12);
    ...
    currentY += lineSpacing;
    doc.text(`Transportador: ${order.nombreChofer?.name || "No asignado"}`, 10, currentY);
    currentY += lineSpacing;
    doc.text(`Numero de pajeros: ${order.NumeroPasajeros}`, 10, currentY);
    currentY += lineSpacing;
    doc.text(`Dirección de clinica: ${order.direccionRecogida}`, 10, currentY);
    currentY += lineSpacing;
    ...
    // Rellenar tabla
    doc.setFont("helvetica", "normal");
    doc.text("Servicio de Transporte de salud", 15, tableY + 17);
    doc.text(
    ...
    // Guardar el archivo
    doc.save(`detalle_Transporte_Salud_${order._id || "orden"}.pdf`);
  };
      ...
      {loading ? (
        ...
        <div>
          <div className="flex justify-between items-center border-b pb-3">
            <h2 className="text-xl font-bold">{order.tipoDeVehiculo}</h2>
    ...
    >
      ...
      <span className="text-xs">Cancelado</span>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-gray-700">Numero de Pasajeros:</span>
              <span>{order.NumeroPasajeros}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <span className="text-gray-700">Dirección de clinica: </span>
...
export default DetalleTransporteSalud