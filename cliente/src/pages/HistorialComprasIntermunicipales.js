...
const HistorialComprasIntermunicipales = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Historial de compras intermunicipales</h1>
    ...
    Iniciar sesión
        ...
        <div>
          {mensajeriaList.length === 0 ? (
            <p>No tienes órdenes de compras intermunicipales.</p>
          ) : (
              ...
              {mensajeriaList.map((order) => (
                <li key={order._id} className="p-6 bg-white bg-opacity-80 border border-gray-300 rounded-lg shadow-md">
                  <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-semibold">{order.ubicacionCompra}</h2>
                    <p className={`text-sm ${order.estado === 'cancelado' ? 'text-red-500' : 'text-green-500'}`}>
                      {order.estado}
                    </p>
                  </div>

                  <p className="text-gray-700">Fecha: {new Date(order.fechaHoraEntrega).toLocaleString()}</p>
...
export default HistorialComprasIntermunicipales